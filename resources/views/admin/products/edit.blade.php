            ...
            <form action="{{route('admin.products.store')}}" method="post" id="add_product_form" name="add_product_form" enctype="multipart/form-data">
                ...
                <div class="row form-group justify-content-center justify-content-md-end">
                    <div class="custom-file col-sm-8 col-md-6 p-0">
                        ...
                        <input type="file" onchange="previewFiles(this.files)" class="custom-file-input" multiple="multiple" id="albumImage" lang="ro" name="albumImage[]">
...
$('#add_product_form').on('submit', function (e) {
    ...
    formData.append('deleteIds[]', deleteIds[i]);
    }
    $.ajax({
        method: 'post',
        processData: false,
        ...
        url: " {{route('admin.products.update', ['product'=>$post]) }}",
        encode: true,
    })
...
function removeExistingImage(event) {
    ...
    const thumbId = thumbDiv.getAttribute('id');
    thumbDiv.remove();
    let photoId =thumbDiv.dataset.id;
    deleteIds.push(photoId);
    // console.log(photoId);
