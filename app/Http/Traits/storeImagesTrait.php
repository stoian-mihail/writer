        ...
        if (isset($input_data['albumImage'])) {
            foreach ($input_data['albumImage'] as $key => $file) {
                if ($file->isValid()) {
                    ...
                    $file_name = $seo_title . now()->format('Y-m-d-H-m-s') . "-" . $key . ".jpg";
                    $thumb_name = $seo_title . now()->format('Y-m-d-H-m-s') . "-" . $key;
                    ...
                    Storage::put("public/$base_directory/{$file_name}", $fileResized->__toString());
                    $fileUrl = "/storage/$base_directory/{$file_name}";
